<?php
class Form_8b_model extends CI_Model {
	...
	public function edit_dates() {
		...
		if ($result == true) {
			...
			$message = "This email is from Drug Control & Pharmacy Services Health Department KP.<br><br>
				...
				Dear Proprieter " . $getProprietor['name'] . ". Your Form 8B certifictae has been approved and Ready to print. Please check details on portal.
	...
	public function getFormTypeDocForm8b($id) {
		$query = $this->db->query('SELECT
		   ...
		   tbl_form_8b_apply_documents.id as tblForm8bApplyDocID,
		   tbl_form_8b_apply_documents.uploaded_document as uploaded_document,
		   tbl_form_8b_apply_documents.document_name as document_name,
		   tbl_form_8b_apply_documents.document_tag_name as document_tag_name,
		   tbl_form_8b_apply_documents.tbl_form_8b_id as from8bID,
		   tbl_form_8b_apply_documents.tbl_form_type_id as tbl_form_8b_apply_documents_fromTypeID,
		   tbl_form_8b_apply_documents.tbl_form_type_doc_id as fromTypeDocID
		FROM
		   tbl_form_type_doc
		   left OUTER JOIN
		   tbl_form_8b_apply_documents  ON tbl_form_8b_apply_documents.tbl_form_8b_id= ' . $id . ' and tbl_form_type_doc.id=tbl_form_8b_apply_documents.tbl_form_type_doc_id and tbl_form_type_doc.tbl_form_type_id= 2 and tbl_form_8b_apply_documents.tbl_form_type_id= 2 where (tbl_form_type_doc.tbl_form_type_id= 2 and tbl_form_type_doc.status= 1 )');
	...
	// public function update_form_8b($form_type_docs, $tbl_form_8b_apply_documents) {
	public function update_form_8b($form_type_docs, $document_name, $tbl_form_type_doc_id, $getPreviousPharmacistID) {
		...
		if ($result == true) {
			...
			$this->db->delete('tbl_form_8b_apply_documents', array('tbl_form_8b_id' => $this->input->post('id')));
			...
			for ($x = 0; $x < $count; $x++) {
				...
				$data = array(
					...
					// 'tbl_form_8b_id' => $last_insert_id,
					'tbl_form_8b_id' => $this->input->post('id'),
				...
				$this->db->insert('tbl_form_8b_apply_documents', $data);
	...
	public function add_form_8b($form_type_docs, $document_name, $tbl_form_type_doc_id) {
		...
		if ($this->db->affected_rows() > 0) {
			...
			$data = array(
				...
				'status_by' => $_SESSION['user_id'],
				'status_date' => date('Y-m-d'),
				'remarks' => '<tr><td>System Message: Applicant Apply for Form 8b on ' . date('d-m-Y') . ', Upload the required documents</tr></td>',
				'assign_to' => '0',
				'assign_date' => date('Y-m-d'),
				'record_add_by' => $_SESSION['user_id'],
				'record_add_date' => date('Y-m-d'),
			...
			////////////email////////
			// $receiver = $result['email'];
			// $getData = $this->common_model->getRecordById($last_insert_id, 'tbl_form_8b');
			...
			foreach ($getUserData as $key => $getUserDataInfo) {
				...
				$subject = 'Drug Control & Pharmacy Services Health Department KP'; //email subject
				$message = "This email is from Drug Control & Pharmacy Services Health Department KP.<br><br>
				Dear DG Drug " . $getUserDataInfo['name'] . ", New Form 8B (Retail Store) application for License request has been received. Please check your dashboard.
	...
	public function update_dg() {
		...
		if ($result == true) {
			...
			if (($this->input->post('status') == '1') && ($this->input->post('assign_to') > '0')) {
				...
				$data = $this->security->xss_clean($data);
				$this->db->where('id', $this->input->post('id'));
				$result = $this->db->update('tbl_form_8b', $data);
				...
				////////////email////////
				// $receiver = $result['email'];
				$getData = $this->common_model->getRecordById($this->input->post('id'), 'tbl_form_8b');
	...
	public function fees_form_8b($fee_recipt) {
		...
		if ($result == true) {
			...
			// $this->db->where('tbl_name', 'tbl_form_8b');
	...
	// Get DataTable data
	function get_form_8b($postData = null) {
		...
		$records = $this->db->get('tbl_form_8b')->result();
		...
		foreach ($records as $record) {
			...
			if ($_SESSION['tbl_role_id'] == '2') {
				$dgBtn = '<a href="' . site_url('form_8b/edit_dg/' . safe_encode($record->id)) . '">
			...
			if ($record->id) {
				$logBtn = '<a href="' . site_url('common/logger/' . safe_encode($record->id) . '/tbl_form_8b') . '">
			...
			if ($record->record_add_by == $_SESSION['user_id'] && $record->is_edit == '1') {
				$EditBtn = '<a href="' . site_url('form_8b/edit_form_8b/' . safe_encode($record->id)) . '">
			...
			if ($record->record_add_by == $_SESSION['user_id'] && $record->is_fees == '1') {
				$FeesBtn = '<a href="' . site_url('form_8b/fees_form_8b/' . safe_encode($record->id)) . '">
			...
			$getDates = $this->global->getRecordByArray($tbl_name = 'tbl_application_renewel', array('tbl_name' => 'tbl_form_8b', 'tbl_name_id' => $record->id));

			if ($record->is_dates == '1' && $_SESSION['tbl_role_id'] == '2') {

				$datesBtn = '<a href="' . site_url('form_8b/edit_dates/' . safe_encode($getDates['id'])) . '">
			                   <button type="button" class="btn btn-sm btn-xs btn-warning"><i class="fa fa-edit"></i> License Validity Dates</button>
			                   </a>';
			} else { $datesBtn = "";}

			// $getInspection = $this->global->getRecordByArray('tbl_inspection', array('tbl_name' => 'tbl_form_8b', 'tbl_name_id' => $record->id));

			if ($record->is_print == '1') {
				$printBtn = '<a href="' . site_url('common/print_license/' . safe_encode('tbl_form_8b') . '/' . safe_encode($record->id) . '') . '">
                      <button type="button" onclick="overlay()" class="btn btn-sm btn-xs btn-success"><i class="fa fa-print"></i> Print License</button>
                      </a>';
			} else { $printBtn = "";}

			// if ($record->is_print == '1') {
			$printPDFBtn = '<a href="' . site_url('common/pdf/' . safe_encode('tbl_form_8b') . '/' . safe_encode($record->id) . '') . '">
