<?php
class Form_8a_model extends CI_Model {
	public function __construct() {
		...
		//////// ajax and ssp////////
		// Set table name
		$this->table = 'tbl_form_8a';
		// Set orderable column fields
		$this->column_order = array(null, null, null, null, null, 'record_add_date');
	...
	public function edit_dates() {
		...
		if ($result == true) {
			...
			$message = "This email is from Drug Control & Pharmacy Services Health Department KP.<br><br>
				...
				Dear Proprieter " . $getProprietor['name'] . ". Your Form 8A certifictae has been approved and Ready to print. Please check details on portal.
	...
	public function getFormTypeDocForm8a($id) {
		$query = $this->db->query('SELECT
		   ...
		   tbl_form_8a_apply_documents.id as tblForm8aApplyDocID,
		   tbl_form_8a_apply_documents.uploaded_document as uploaded_document,
		   tbl_form_8a_apply_documents.document_name as document_name,
		   tbl_form_8a_apply_documents.document_tag_name as document_tag_name,
		   tbl_form_8a_apply_documents.tbl_form_8a_id as from8aID,
		   tbl_form_8a_apply_documents.tbl_form_type_id as tbl_form_8a_apply_documents_fromTypeID,
		   tbl_form_8a_apply_documents.tbl_form_type_doc_id as fromTypeDocID
		FROM
		   tbl_form_type_doc
		   left OUTER JOIN
		   tbl_form_8a_apply_documents  ON tbl_form_8a_apply_documents.tbl_form_8a_id= ' . $id . ' and tbl_form_type_doc.id=tbl_form_8a_apply_documents.tbl_form_type_doc_id and tbl_form_type_doc.tbl_form_type_id= 1 and tbl_form_8a_apply_documents.tbl_form_type_id=1 where (tbl_form_type_doc.tbl_form_type_id= 1 and tbl_form_type_doc.status= 1 )');
	...
	// public function update_form_8a($form_type_docs, $tbl_form_8a_apply_documents) {
	public function update_form_8a($form_type_docs, $document_name, $tbl_form_type_doc_id, $getPreviousPharmacistID) {
		...
		if ($result == true) {
			...
			$this->db->delete('tbl_form_8a_apply_documents', array('tbl_form_8a_id' => $this->input->post('id')));
			...
			for ($x = 0; $x < $count; $x++) {
				...
				$data = array(
					...
					// 'tbl_form_8a_id' => $last_insert_id,
					'tbl_form_8a_id' => $this->input->post('id'),
				...
				$this->db->insert('tbl_form_8a_apply_documents', $data);
	...
	public function add_form_8a($form_type_docs, $document_name, $tbl_form_type_doc_id) {
		...
		$data = array(
			...
			'record_add_by' => $_SESSION['user_id'],
			'record_add_date' => date('Y-m-d'),
			// 'tracking_code' => '8a - ' . random_string('alnum', 4),
		...
		if ($this->db->affected_rows() > 0) {
			...
			$data = array(
				...
				'status_by' => $_SESSION['user_id'],
				'status_date' => date('Y-m-d'),
				'remarks' => '<tr><td>System Message: Applicant Apply for Form 8a on ' . date('d-m-Y') . ', Upload the required documents</tr></td>',
				'assign_to' => '0',
				'assign_date' => date('Y-m-d'),
				'record_add_by' => $_SESSION['user_id'],
				'record_add_date' => date('Y-m-d'),
			...
			////////////email////////
			// $receiver = $result['email'];
			// $getData = $this->common_model->getRecordById($last_insert_id, 'tbl_form_8a');
			...
			foreach ($getUserData as $key => $getUserDataInfo) {
				...
				$subject = 'Drug Control & Pharmacy Services Health Department KP'; //email subject
				$message = "This email is from Drug Control & Pharmacy Services Health Department KP.<br><br>
				Dear DG Drug " . $getUserDataInfo['name'] . ", New Form 8A (Pharmacy) application for License request has been received. Please check your dashboard.
	...
	public function update_dg() {
		...
		if ($result == true) {
			...
			if (($this->input->post('status') == '1') && ($this->input->post('assign_to') > '0')) {
				...
				$data = $this->security->xss_clean($data);
				$this->db->where('id', $this->input->post('id'));
				$result = $this->db->update('tbl_form_8a', $data);
				...
				////////////email////////
				// $receiver = $result['email'];
				$getData = $this->common_model->getRecordById($this->input->post('id'), 'tbl_form_8a');
	...
	public function fees_form_8a($fee_recipt) {
		...
		if ($result == true) {
			...
			// $this->db->where('tbl_name', 'tbl_form_8a');
			...
			foreach ($getUserData as $key => $getUserDataInfo) {
				...
				$subject = 'Drug Control & Pharmacy Services Health Department KP'; //email subject
				$message = "This email is from Drug Control & Pharmacy Services Health Department KP.<br><br>
				Dear DG Drug " . $getUserDataInfo['name'] . ", Fees is Payed by Applicant for License and request has been received. Please check your dashboard for further proceedings.
	...
	// Get DataTable data
	function get_form_8a($postData = null) {
