...
class Form_8d extends MY_Controller {
	...
	public function view_form_8d() {
		$data['district'] = $this->common_model->getAllRecordByArray('tbl_district', array('status' => '1'));

		$data['page_title'] = 'form 8D - Narcotics applications';
		$data['description'] = '...';
		$this->load->view('templates/header', $data);
		$this->load->view('form_8d/view_form_8d', $data);
		$this->load->view('templates/footer');
	}

	public function add_form_8d() {

		$data['page_title'] = 'license form 8D - Narcotics';
		$data['description'] = 'Apply license for Narcotics ';
		...
		//form type ( form 8d Database ID is 1)
		$data['form_type_doc'] = $this->common_model->getAllRecordByArray('tbl_form_type_doc', array('status' => '1', 'tbl_form_type_id' => '4'));
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() === FALSE) {
				$this->load->view('templates/header', $data);
				$this->load->view('form_8d/add_form_8d');
				$this->load->view('templates/footer');
			} else {
				...
				foreach ($getDoc as $key => $getDocInfo) {
					...
					if ($fileUpload->uploaded == true) {
						...
						$fileUpload->process('./assets/upload/images/form_8d');
				...
				// to model
				$this->form_8d_model->add_form_8d($form_type_docs, $document_name, $tbl_form_type_doc_id);
				// set session message
				$this->session->set_flashdata('add', '!');
				redirect(base_url('view_form_8d'));
			}
	...
	// 	$data = $this->form_8d_model->getRecordById($id);
	...
	public function get_form_8d() {
		// POST data
		$postData = $this->input->post();

		// Get data
		$data = $this->form_8d_model->get_form_8d($postData);
	...
	public function edit_dates($id = null) {
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$this->load->view('templates/header', $data);
				$this->load->view('form_8d/edit_dates');
				$this->load->view('templates/footer');
			} else {

				$this->form_8d_model->edit_dates();
	...
	public function edit_form_8d($id = null) {
		$id = safe_decode($id);
		if ($this->input->post('submit')) {
			$this->form_validation->set_rules('tbl_form_type_id', 'Selection', 'required|xss_clean');
			$this->form_validation->set_rules('tbl_proprietor_id', 'Selection', 'required|xss_clean');
			...
			$getDoc = $this->form_8d_model->getFormTypeDocForm8d($this->input->post('id'));
			foreach ($getDoc as $key => $getDocInfo) {
				// echo '<pre>';
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$data['all'] = $this->common_model->getRecordById($this->input->post('id'), 'tbl_form_8d');
				...
				$this->load->view('templates/header', $data);
				$this->load->view('form_8d/edit_form_8d');
				$this->load->view('templates/footer');
			} else {
				...
				foreach ($getDoc as $key => $getDocInfo) {
					//tbl_form_8d_apply_documents is the id of tbl_applicant_documents table
					...
					if (!empty($_FILES[$getDocInfo['tag_name']]['name'])) {
						...
						if ($fileUpload->uploaded == true) {
							...
							if ($fileUpload->processed == true) {
								...
								$imageName = $this->common_model->getImageNameById($this->input->post('tbl_form_8d_apply_documents' . $getDocInfo['tblForm8dApplyDocID']), 'tbl_form_8d_apply_documents');

								//del the image from folder

								$this->common_model->delImage($folderName = 'form_8d', $imageName['uploaded_document']);
				...
				$getPreviousPharmacistID = $this->common_model->getRecordById($this->input->post('id'), 'tbl_form_8d');
				// $getPharmacistData = $this->common_model->getRecordById($getPharmacistID['tbl_pharmacist_id'], 'tbl');

				$this->form_8d_model->update_form_8d($form_type_docs, $document_name, $tbl_form_type_doc_id, $getPreviousPharmacistID);
				$this->session->set_flashdata('updated', '!');
				redirect(base_url('view_form_8d'));
			}
		} else {
			$data['all'] = $this->common_model->getRecordById($id, 'tbl_form_8d');
			...
			$data['page_title'] = 'license form 8D - Narcotics';
			$data['description'] = 'Edit license for Narcotics ';
			if (empty($data['all'])) {
				show_404();
			}
			$this->load->view('templates/header', $data);
	...
	public function fees_form_8d($id = null) {
		$id = safe_decode($id);
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$this->load->view('templates/header', $data);
				$this->load->view('form_8d/fees_form_8d');
				$this->load->view('templates/footer');
			} else {
				...
				if (!empty($_FILES['fee_recipt']['name'])) {
					...
					if ($fileUpload->uploaded == true) {
						...
						if ($fileUpload->processed == true) {
							//del the previous/old image befor the update new one
							// get image name from directory against record id
							$imageName = $this->common_model->getImageNameById($this->input->post('id'), 'tbl_form_8d');
							//del the image from folder
							...
							$this->session->set_flashdata('img_upload_error', $fileUpload->error);
							redirect(base_url('fees_form_8d'));
						}
					} else {
						$this->session->set_flashdata('img_upload_error', $fileUpload->error);
						redirect(base_url('fees_form_8d'));
				...
				$this->form_8d_model->fees_form_8d($fee_recipt);
				$this->session->set_flashdata('updated', '!');
				redirect(base_url('view_form_8d'));
			}
		} else {
	...
	public function edit_dg($id = null) {
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$this->load->view('templates/header', $data);
				$this->load->view('form_8d/edit_dg');
				$this->load->view('templates/footer');
			} else {

				$this->form_8d_model->update_dg();
				$this->session->set_flashdata('updated', '!');
				redirect(base_url('view_form_8d'));
			}
		} else {
	...
	public function fetchBankBranchesByBankID($tbl_bank_id) {
		$data = $this->form_8d_model->fetchBankBranchesByBankID($tbl_bank_id);
		echo json_encode($data);
	}
}
?>
